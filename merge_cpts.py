    ...
    for line in f1:
        ...
        for line2 in weights:
            if str(line2[0][0]) == str(word[0]):
                tempStat = np.array([(word[0], (float(word[4]) * line2[0][1] * line2[0][2]), (float(word[5]) * line2[0][1]))], dtype=t)
    ...
    for line in stats:
        ...
        for workload in result:
            if str(workload[0][0]) == str(cpt[0]):
                workload[0] = np.array([(cpt[0], workload[0][1] + line[0][1], workload[0][2] + line[0][2])], dtype=t)
                break
