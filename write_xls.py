...
def write_xls():
    ...
    for i in range(0, 3):
        ...
        if i == 0:
            read_file = 'decoupled_loop_stats.txt'
            graph_name = 'decoupled_loop'
        elif i == 1:
        ...
        if i == 0:
            decoupled_loop_misRate = misRate
            decoupled_loop_mpki = mpki
        elif i == 1:
    ...
    for i in range(len(decoupled_mpki)):
        sheet.append([str(workload_name[i]), str(decoupled_loop_mpki[i]), str(decoupled_mpki[i]), str(coupled_mpki[i])])
    ...
    for i in range(len(decoupled_misRate)):
        sheet2.append([str(workload_name[i]), str(decoupled_loop_misRate[i]), str(decoupled_misRate[i]), str(coupled_misRate[i])])
